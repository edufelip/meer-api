...
public class AdminCategoryController {
    ...
    var items =
        pageRes.getContent().stream()
            .map(
                c ->
                    new CategoryDto(
                        c.getId(), c.getNameStringId(), c.getImageResId(), c.getCreatedAt()))
            .toList();
  ...
  private void validateBody(CategoryUpsertRequest body, boolean requireId, String pathId) {
    ...
    if ((requireId && (body.id() == null || body.id().isBlank()))
        || (pathId != null && (body.id() == null || !body.id().equals(pathId)))) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "id is required and must match path");
    }
